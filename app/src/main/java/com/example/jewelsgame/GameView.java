...
public class GameView extends SurfaceView implements SurfaceHolder.Callback {
        ...
        for (int i = 0; i < board.length; i++) {
            for (int j = 0; j < board[0].length - 2; j++) {
                if (board[i][j].color > 0 &&
                        ...
                        board[i][j + 1].color == board[i][j + 2].color) {
        ...
        for (int j = 0; j < board[0].length; j++) {
            for (int i = 0; i < board.length - 2; i++) {
                if (board[i][j].color > 0 &&
                        ...
                        board[i + 1][j].color == board[i + 2][j].color) {
                    search.add(new ArrayList<>());
                    search.get(search.size() - 1).add(new Point(i, j));
    ...
    public void draw(Canvas canvas){
        ...
        if (canvas != null) {
            ...
            canvas.drawText("Moves: " + remainingMoves, 50, 200, paint);
        }
    }
                ...
                if(gameState==GameState.nothing){
                    ...
                    if(move&&(deltaX>30 || deltaY>30)){
                        ...
                        if(Math.abs(oldX-newX)>Math.abs(oldY- newY)){
                            ...
                            newPoseI=poseI;
                        }else if (Math.abs(oldY-newY)>Math.abs(oldX-newX)){
                            if (newY>oldY){
                                direction="down";
                                newPoseI=poseI+1;
                            }else{
                        ...
                        if(newPoseI >= 0 && newPoseI < board.length && newPoseJ >= 0 && newPoseJ < board[0].length) {
    ...
    private void endGame() {
        gameEnded = true;
        dbHelper.addScore(userId, score);
        Toast.makeText(getContext(), "Oyun Bitti! Skorunuz: " + score, Toast.LENGTH_LONG).show();
        Intent intent = new Intent(getContext(), Scores.class);
